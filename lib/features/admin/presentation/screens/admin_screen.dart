...
class _AdminScreenState extends ConsumerState<AdminScreen>
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final users = await adminSvc.fetchAllUsers();
      final subs = await adminSvc.getSubscriptionRequests();
      final promos = await adminSvc.listPromos();
      final banned = await adminSvc.listBanned();

      if (!mounted) return;

      setState(() {
        _users = List<UserModel>.from(users);
        _subRequests = List<Map<String, dynamic>>.from(
          (subs as List).map<Map<String, dynamic>>(
            (s) => Map<String, dynamic>.from(s.toMap()),
          ),
        );
        _promos = List<Map<String, dynamic>>.from(
          (promos as List).map<Map<String, dynamic>>(
            (p) => Map<String, dynamic>.from(p),
          ),
        );
        _banned = List<Map<String, dynamic>>.from(
          (banned as List).map<Map<String, dynamic>>(
  ...
  Widget build(BuildContext context) {
    ...
    if (!(currentUser?.isAdmin ?? false)) {
      return Scaffold(
        appBar: AppBar(title: Text(loc.admin)),
        body: const Center(
          child: Text(
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: [
            ...
            const Tab(text: 'البروموكود'),
            const Tab(text: 'الحظر'),
          ],
        ),
      ),
      body: _loading
    ...
    return PopupMenuButton(
      ...
      itemBuilder: (_) => [
        if (user.isPending) ...[
          const PopupMenuItem(
            value: 'approve',
            child: Text('موافقة', style: TextStyle(fontFamily: 'Cairo')),
          ),
          const PopupMenuItem(
            value: 'reject',
            child: Text(
        ...
        const PopupMenuItem(
          value: 'force_logout',
          child: Text('إخراج إجباري', style: TextStyle(fontFamily: 'Cairo')),
        ),
        const PopupMenuItem(
          value: 'ban_chat',
          child: Text(
      ...
      onSelected: (v) async {
        ...
        switch (v) {
          case 'approve':
            await adminSvc.approveUser(user.uid, true);
            break;
          case 'reject':
            await adminSvc.approveUser(user.uid, false);
            break;
          case 'force_logout':
            await adminSvc.forceLogoutUser(user.uid);
            break;
          case 'ban_chat':
            await adminSvc.banUserFromChat(user.uid, !user.chatBanned);
            break;
          case 'delete':
            await adminSvc.deleteUser(user.uid);
            break;
        }
        await _loadData();
      },
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text(
                          'خصم: ${p['discount']}% • استخدامات: ${p['usedCount']}/${p['maxUses']}',
                          style: const TextStyle(
                            fontFamily: 'Cairo',
                            fontSize: 12,
                          ),
      ...
      children: [
        Padding(
          ...
          child: OutlinedButton.icon(
            onPressed: () async {
              final ok = await ref.read(adminServiceProvider).forceLogoutAll();
              if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(ok ? 'تم إخراج الجميع' : 'فشل'),
                    backgroundColor:
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            TextField(
              controller: maxCtrl,
              decoration: const InputDecoration(labelText: 'أقصى استخدام'),
              keyboardType: TextInputType.number,
            ),
          ],
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              await ref.read(adminServiceProvider).createPromo(
                    codeCtrl.text.trim(),
                    double.tryParse(discCtrl.text) ?? 10,
